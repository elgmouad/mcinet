...
function ListMissions({ role, user }) {
  ...
  const handleMissionStatus = (id, Id_object) => {
    const roleOfStatus = role === 'CADRE' ? "Validé" : "En Cours"

    if (role === 'CADRE') {
      navigate('/dashboard/orderMissions/control/add', { state: { id: id, objectId: Id_object } })
    } else {
                    ...
                    <div className='absolute top-6 right-12 z-50 shadow-lg bg-[#E4E4E4] rounded-[12px] overflow-hidden' ref={missionMenuRef}>
                      {
                        role !== "CADRE" ? (
                          <div>
                            <p onClick={() => handleMissionStatus(mission.mission_id,mission.Id_object)} className='min-h-fit !py-2 !px-4 rounded-[10px] hover:bg-bg-blue hover:text-blue cursor-pointer'>Attribuer</p>
                          ...
                          : (
                            <p onClick={() => handleMissionStatus(mission.mission_id, mission.Id_object)} className='min-h-fit !py-2 !px-4 rounded-[10px] hover:bg-bg-blue hover:text-blue cursor-pointer'>Executer</p>
                          )
                      }
                    </div>
                  )}
